...
- In the Compact layout, ref labels render right-aligned inside the summary's cell, capped at roughly a third of that cell; labels that overflow the cap are clipped at the cell edge, and hovering the labels reveals every label in full. The summary truncates with an ellipsis before the labels give up space.
