...
The user sees a graphical history of the repository's commits with branch lanes and merge connectors. The history covers the checked-out commit, every local branch, every remote-tracking branch, and every tag, so work that has not been merged or pulled — and releases whose branches are gone — is visible alongside the checked-out history. Commits from all branches interleave in a single graph ordered newest-first, with no visual distinction between commits that are and are not part of the checked-out history. Branch and merge lines use smooth rounded bends where they turn between lanes, including the final turn into a branch commit. Each commit is presented in one of two selectable layouts, and a segmented control in the selection bar switches between them with the choice remembered across sessions. In the Compact layout the commit is a single-row entry whose reading order is graph, summary line, ref labels, short identifier, author, and authored time; the summary and the ref labels share one flexible cell — summary at the left edge, labels right-aligned — growing toward each other: the labels may use whatever width the summary's text leaves free, and when both want more than the cell holds the summary is guaranteed the greater share before it truncates. Above the rows, Compact mode pins a header naming the columns; the author and time columns can be resized by dragging the dividers between the header's labels, and those widths persist across sessions. The authored time reads as a compact relative form ("2h", "3d") rather than a date. In the Table layout the row is taller and its metadata splits across two lines beside the graph: the summary alone on the first line, free to use the row's full width, and the short identifier, author, and authored date on the left of the second line with the ref labels right-aligned on the right, the two groups growing toward each other. In both layouts the ref labels are any branch names or tags — local branch, remote-tracking branch, or tag — that point at the commit, and hovering them reveals the full set, unabridged, in a tooltip that also names each label's role — checked out, checked out in a linked worktree, remote, or tag. In the Compact layout a label never renders partially cut off: labels that do not fit collapse into a count badge instead; in the Table layout labels that do not fit are clipped. The graph is scrollable; older commits load progressively as the user scrolls. The currently checked-out tip is visually marked, but selection is independent of checkout state — reviewing never moves HEAD.
...
- Commits reachable only from unmerged local branches, remote-tracking branches, or tags appear
  ...
  ends at its tip.
- Each visible commit shows its short identifier, summary line, author, and when it was authored — a date in the Table layout, a compact relative time in the Compact layout.
...
- In the Compact layout, ref labels render right-aligned inside the summary's cell and grow into whatever width the summary's text leaves free; a short summary lets the labels use most of the cell. When the summary and the labels together want more width than the cell holds, the summary is guaranteed roughly the greater three-fifths of the cell and truncates with an ellipsis only beyond that guarantee.
- In the Compact layout, a label never renders partially cut off: as many whole labels as fit are shown, in order, and the remainder collapse into a "+N" count badge at the end of the cluster. Hovering the badge lists exactly the hidden labels; hovering the cluster still reveals every label in full. Even the narrowest cell keeps the commit's first label — ellipsized if necessary — beside the badge, so a commit's refs never disappear entirely.
- In the Compact layout, a pinned header row labels the columns (summary and refs, commit identifier, author, authored time) and never scrolls with the rows; the Table layout has no header row. Dragging the divider at the left edge of the author or time column resizes that column between fixed bounds, and the widths are restored on the next launch.
- In the Compact layout, the pending-changes row fills the metadata columns with placeholders: a dash for the identifier and an italic "working tree" for the author, with the time column empty.
- In the Table layout, each commit occupies a taller two-line row: the summary alone on the first line, and the short identifier, author, and authored date on the left of the second line with the ref labels right-aligned on the right.
- A segmented control in the selection bar switches the graph between the Compact and Table layouts; the selected layout is remembered across sessions. The open-changeset affordance sits at the left of the selection bar, next to the selection summary.
