...
- In the Compact layout, ref labels render in a shared leading column sized to the widest visible label set, capped at roughly a third of the panel width; labels that overflow the cap are clipped at the column edge, and hovering the column reveals every label in full. The column is the same width on every row, and it yields its space to the commit summary when no labels are visible.
